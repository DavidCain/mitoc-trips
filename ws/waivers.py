...
def _get_system_to_system_jwt() -> str:
    token = _get_possibly_cached_access_token()
    last_known_exp: int = jwt.decode(token, options={"verify_signature": False})["exp"]
    if time.time() - 30 > last_known_exp:
        # If expired, this *must* have been a cache hit on an old token...
        assert _get_possibly_cached_access_token.cache_info().currsize > 0, (
            "Fresh token is already expired!?"
        )
        _get_possibly_cached_access_token.cache_clear()
        return _get_possibly_cached_access_token()
    return token


# A janky TTL lets us call this method repeatedly w/o passing around a token.
# (using an in-memory JWT also makes user flows faster)
@functools.cache  # Will manually bust
def _get_possibly_cached_access_token() -> str:
...
def initiate_waiver(
    ...
    env = requests.post(
        f"{get_base_url()}/envelopes", json=new_env, headers=get_headers(), timeout=10
    )
    ...
    if participant:  # We need a participant to do embedded signing
        ...
        redir_url = _sign_embedded(participant, releasor_dict, envelope_id)
